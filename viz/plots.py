...
def make_clickable_track_fig(gps):
    df = gps.sort_values("timestamp") if "timestamp" in gps.columns else gps
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="lat=%{y:.6f}<br>lon=%{x:.6f}<extra></extra>",
    ))
...
def add_gate_marker(fig, gate_center):
    lat, lon = gate_center
    ...
    fig.add_trace(go.Scatter(
        ...
        hovertemplate="GATE<br>lat=%{y:.6f}<br>lon=%{x:.6f}<extra></extra>",
    ))
